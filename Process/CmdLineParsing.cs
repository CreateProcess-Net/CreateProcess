...
using System.Text;
  ...
  {
    internal static string escapeCommandLineForShell(string cmdLine) => $"'{cmdLine.Replace("'", "'\\''")}'";
        ...
        while (enumerator.MoveNext())
        {
          ...
          while ((startIndex >= current.Length ? 0 : (num >= 0 ? 1 : 0)) != 0)
          {
            ...
            if (num >= 0)
            {
              ...
              CmdLineParsing.escapeBackslashes(sb, current, num - 1);
              stringBuilder = sb.Append('\\');
              stringBuilder = sb.Append('"');
              startIndex = num + 1;
            }
          ...
          CmdLineParsing.escapeBackslashes(sb, current, current.Length - 1);
          stringBuilder = sb.Append("\" ");
        }
      }
      finally
    ...
    internal static string[] windowsCommandLineToArgv(string arguments)
    {
      if (arguments.Contains("\"\"\""))
        throw new ArgumentException($"triple quotes are not allowed in the command line ('{arguments}') as they behave different across programs, see https://github.com/vbfox/FoxSharp/issues/1 to escape a quote use backslash and the rules from https://docs.microsoft.com/en-US/cpp/cpp/parsing-cpp-command-line-arguments?view=vs-2017.", nameof (arguments));
    ...
    {
      string commandLine = CmdLineParsing.windowsArgvToCommandLine(true, args);
...
public static class Args
{
  ...
  /// </summary>
  public static string toWindowsCommandLine(IEnumerable<string> args) => CmdLineParsing.windowsArgvToCommandLine(true, args);

  /// <summary>Escape the given argument list according to a unix shell (bash)</summary>
  public static string toLinuxShellCommandLine(IEnumerable<string> args) => string.Join(" ", args.Select(CmdLineParsing.escapeCommandLineForShell));

  /// <summary>Read a windows command line string into its arguments</summary>
  public static string[] fromWindowsCommandLine(string cmd) => CmdLineParsing.windowsCommandLineToArgv(cmd);
}
...
public record Arguments
{
  ...
  public static Arguments Empty { get; } = new Arguments(Array.Empty<string>(), null);
  /// <summary>
  /// See https://msdn.microsoft.com/en-us/library/17w5ykft.aspx
  /// </summary>
  public static Arguments OfWindowsCommandLine(string cmd) => new Arguments(Process.Args.fromWindowsCommandLine(cmd), cmd);
  /// <summary>
  /// Create a new arguments object from the given list of arguments
  /// </summary>
  public static Arguments OfArgs(IEnumerable<string> args) => new Arguments(args, null);
  /// <summary>
  /// Create a new arguments object from a given startinfo-conforming-escaped command line string.
  /// Same as `OfWindowsCommandLine`.
  /// </summary>
  public static Arguments OfStartInfo (string cmd) => OfWindowsCommandLine(cmd);

  /// <summary>
  /// This is the reverse of https://msdn.microsoft.com/en-us/library/17w5ykft.aspx
  /// </summary>
  public string WindowsCommandLine => Process.Args.toWindowsCommandLine(Args);

  /// <summary>
  /// Escape the given argument list according to a unix shell (bash)
  /// </summary>
  public string LinuxShellCommandLine => Process.Args.toLinuxShellCommandLine(Args);
...
public static class ArgumentsExtensions
{
  ...
  /// </summary>
  public static Arguments ToArguments(this string cmd) => Arguments.OfWindowsCommandLine(cmd);
  
  /// <summary>
  /// Append the given arguments before all current arguments 
  /// </summary>
  public static Arguments WithPrefix(this Arguments a, IEnumerable<string> s) => Arguments.OfArgs(s.Concat(a.Args));
  /// <summary>
  /// Append all arguments after the current arguments    
  /// </summary>
  public static Arguments Append(this Arguments a, IEnumerable<string> s) => Arguments.OfArgs(a.Args.Concat(s));
  /// <summary>
  ...
  public static IReadOnlyList<string> ToList(this Arguments a) => a.Args;
}